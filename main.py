...
class MainLayout(BoxLayout):
    def __init__(self,**kwargs):
        ...
        for child in self.more_inputs:
            if child == "inv":
                ...
                self.more_layout.add_widget(btn)
                if btn.text in self.tri or btn.text in self.tri_inverse:
                    btn.bind_event('on_release',partial(self.insert,child))
                    self.buttons.append(btn)
                    print(f'callbacks : {self.callbacks}')
                else:
                    btn.bind(on_press=partial(self.insert,child))
                    
        
        button_order = [7, 8, 9, 4, 5, 6, 1, 2, 3, 0, -1, -2]
    ...
    def modify(self, input):
        ...
        input = input.replace('sin-¹(', 'asin(')
        input = input.replace('cos-¹(', 'acos(')
        input = input.replace('tan-¹(', 'atan(')
        ...
        print(f'Final expression for AST : {input}')
        return input
    ...
    def insert(self, *args):
        ...
        try:
            ...
            if self.check.check_brackets(evaluated_text):
                ...
                self.cresult = AST.safe_eval(evaluated_text)
    ...
    def delete(self):
        if len(self.text_input.text_input.text) > 0:
            if self.reset_input:
                self.text_input.text_input.text = ""
            elif self.text_input.text_input.text == '(':
                self.text_input.text_input.text = self.text_input.text_input.text[:-1]
                self.text_input.text_input.scroll_x = 5
    ...
    def execute(self, instance):
        ...
        try:
            ...
            final_result = AST.safe_eval(input_expression)
            ...
            if float(final_result).is_integer():
                ...
                self.text_input.text_input.text = str(self.dynamic_round(final_result))
    ...
    def dynamic_round(self,num):
        ...
        return round(num, min(decimal_places, max_decimal_places))
            ...
            for child in self.more_layout.children:  
                ...
                for j in self.tri:
                    if child.text == j:
                         ...
                         child.bind_event('on_release',partial(self.insert, ctext))
...
class Manager(ScreenManager):
    def __init__(self,**kwargs):
        ...
        self.transition = CardTransition(duration=0.5)
...
class SettingScreen(Screen):
    ...
    def show_about(self,instance):     
        ...
        popup = Popup(title="About",content=layout,size_hint=(1,1),auto_dismiss=False)
        popup.open()
        
        layout.add_widget(Label(text="""
A modern and intuitive scientific
calculator with advanced features like
calculation history, customizable themes,
and a clean interface for all your
 mathematical needs.

Note: Calculations may be delayed if the
expression is incomplete. Try closing any
open brackets or pressing '=' to force a
result.

Long-press the 'Del' button to clear all input.

App Version: 1.0.0""",size_hint=(1,.5),color=(0,0,0,1),bold=True,font_size=30,pos_hint={"y":.7,"x":.01}))
        btn = Button(
        text="ok",
        size_hint=(None, None),
        size=(300, 100),
        ...
        donate_button.bind(on_press=self.open_donation_link)
...
class MyFirstScreen(Screen):
    def __init__(self,**kwargs):
            ...
            #search image
            image_path = resource_find('assets/images/optimal_logo.jpg')
        
            optimal_logo = Image(source=image_path,size_hint=(1,.7),pos_hint={'center_y':.5,'center_x':.5})
...
class ModernCalculator(App):
    def build(self):
        try:
            ...
            return Label(text="An error occurred. Check the log for details.")
