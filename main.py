...
long press the ''Del'' button to clear input""",size_hint=(1,.5),color=(0,0,0,1),bold=True,font_size=30,pos_hint={"y":.7,"x":.01}))
        btn = Button(text="ok",size_hint=(.4,.1),pos_hint={"y":.3})
...
class ModernCalculator(App):
    def build(self):
        try:
            ...
            # This will print the full error traceback to the log
            print("----------------- APPLICATION CRASHED -----------------")
            traceback.print_exc()
            print("-----------------------------------------------------")
            
            # This is optional, but it will keep the app from crashing immediately
